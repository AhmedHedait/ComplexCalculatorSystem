...
int i;
	...
	{
	NVIC_ST_RELOAD_R = delay_us * 80;									// number of counts to wait, 80 ticks = 1 us (at 80Mhz)
...
/********************************************************************************************/
	...
	void SysTick_Wait_ms(unsigned int delay_ms) 
		{
	//int i;
//s	for( int i=0; i<delay_ms; i++)
		{
		NVIC_ST_RELOAD_R = 80000;  			// number of counts to wait, 80000 ticks = 1 ms (at 80Mhz)
		NVIC_ST_CURRENT_R = 0;  										// Clear CURRENT counter value
		while((NVIC_ST_CTRL_R & 0x00010000)==0){} 	// wait for count flag
		}
		}
...
/********************************************************************************************/
	...
	void SysTick_Wait_ns(unsigned int delay_ns)
		{
	//for( int i=0; i<delay_ns; i++){
		NVIC_ST_RELOAD_R = 1;  			// number of counts to wait, 1 ticks = 12.5 ns (at 80Mhz)
		NVIC_ST_CURRENT_R = 0;  										// Clear CURRENT counter value
		while((NVIC_ST_CTRL_R & 0x00010000)==0){} 	// wait for count flag
		}
